...
public class Trie_map {
	...
	public String search_LCP(String word) {
		...
		for(int i=0;i<word.length();i++) {
			...
			char cur_letter=word.charAt(i);
			
			if(node_cur.next.containsKey(cur_letter)&&node_cur.next.size()==1 && !node_cur.isWord) {
				
				prefix.append(cur_letter);
				
				node_cur=node_cur.next.get(cur_letter);
			}
	...
	public int Search_LCP(String word) {
