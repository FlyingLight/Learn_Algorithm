...
public class BT_imp<K extends Comparable<K>,V> implements BT<K,V>{
	...
	public void postorder() {
		...
		private int sum_node_num(Node node_cur){
			...
			if(node_cur!=null && node_cur.left_child==null && node_cur.right_child==null)
				
				return 1;

			int left=sum_node_num(node_cur.left_child);

			int right=sum_node_num(node_cur.right_child);
		...
		public int sum_node_val() {
			
			if(node_root== null) return 0;
			
			else {
				
				int sum =_sum_node_val(node_root,0);
		...
		private int _sum_node_val(Node node_cur,int sum) {
			
			if(node_cur==null ) return 0;
			
			if(node_cur!=null && node_cur.left_child==null && node_cur.right_child==null) {
				
				return (int)node_cur.key;
			}
			
			int left_sum =_sum_node_val(node_cur.left_child,sum);
			
			int right_sum=_sum_node_val(node_cur.right_child,sum);
			
			sum=left_sum+right_sum+(int)node_cur.key;
		...
		public void flattenTree(Node node) {
			...
			while(node!=null) {
				...
				if(node.left_child==null) {
					...
					node_cur.right_child=node.right_child;
	...
	//		System.out.println(Age);
		...
		//	bt_imp.remove(22);
			...
			System.out.println("sum nodes="+" "+bt_imp.countNodes());
			
			System.out.println("sum val="+" "+bt_imp.sum_node_val());
